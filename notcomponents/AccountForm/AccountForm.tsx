...
import { passwordConfirmationValidation, passwordValidation, phoneNumberValidation, sirenValidation, zipCodeValidation } from '../../shared/validations';
...
const passwordValidationSchema = Yup.object().shape({
	oldPassword: Yup.string().required("Une formalité pour être sûr qu'il s'agit bien de vous"),
...
class AccountForm extends React.Component<IAccountFormProps> {
	...
	public render() {
		...
		const updateInfoFormComponent = () => (
			<Form autoComplete="off">
				<Grid container className={classes.grid}>
					<Grid item xs={12}>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="username" label="Nom d'utilisateur" name="username" className={classes.textField} margin="normal" disabled />
						</Grid>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="email" label="Email" name="email" className={classes.textField} margin="normal" disabled />
						</Grid>
						<Grid container className={classes.block}>
							<Grid item xs={2}>
								<Field id="gender" name="gender" component={Select} className={classes.genderSelect}>
							...
							<Grid item xs={5}>
								<Field id="firstname" name="first_name" label="Prénom" className={classes.textField} margin="normal" type="text" component={TextField} />
							</Grid>
							<Grid item xs={5}>
								<Field id="lastname" name="last_name" label="Nom" className={classes.textField} margin="normal" type="text" component={TextField} />
							</Grid>
						</Grid>
						<Grid item xs={12}>
							<Field
						...
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="phone_number" label="Numéro de téléphone" name="phone_number" className={classes.textField} margin="normal" />
						</Grid>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="description" label="Description" name="description" className={classes.textField} margin="normal" />
						</Grid>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="address" label="Adresse" name="address" className={classes.textField} margin="normal" />
						</Grid>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="city" label="Ville" name="city" className={classes.textField} margin="normal" />
						</Grid>
						<Grid item xs={12}>
							<Field type="text" component={TextField} id="zipCode" label="Code postal" name="zip_code" className={classes.textField} margin="normal" />
						</Grid>
						<Grid item xs={12}>
		...
		const { username, gender, first_name, last_name, email, description, birthdate, phone_number, address, city, zip_code } = currentGourmet;
				...
				{({ loading, error, data }) => {
					...
					return (
						<div className={classes.container}>
							<Card className={classes.card}>
								<CardContent>
									<ProfileImageUploader imageKey={get(currentGourmet, 'image.key')} identityId={currentGourmet.identity_id} />
									<Formik
										initialValues={{
											username,
											gender,
	...
	public onNewInfoSubmit(values: IUpdateInfoFormValues, { setSubmitting, setErrors, setStatus }) {
		const { currentGourmet, setCurrentGourmet, openSnackbar, updateGourmet } = this.props;

		const { description, gender, first_name, last_name, birthdate, phone_number, address, city, zip_code } = values;
	...
	public onNewCookInfoSubmit(values: IUpdateCookInfoFormValues, { setSubmitting, setErrors, setStatus }) {
		const { currentGourmet, openSnackbar, updateCook } = this.props;
		const { is_pro, description, business_name, siren, pro_email, pro_phone_number, legal_first_name, legal_last_name, legal_birthdate } = values;
	...
	public onNewPasswordSubmit(values: IUpdatePasswordFormValues, { setSubmitting, setErrors, setStatus, resetForm }) {
...
const enhance = compose(graphql(UpdateCook), graphql(UpdateGourmet), withStyles(styles as any));
