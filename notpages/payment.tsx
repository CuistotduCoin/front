		...
		return (
			<Layout headerProps={{ static: true, hideCompanyIndividual: true }}>
				<Grid container justify="space-around" alignItems="center" className={classes.grid}>
					...
					<Grid item>
						<Typography variant="h6">Paiement - {steps[this.state.activeStep]}</Typography>
					</Grid>
					<Grid item>
				...
				{this.state.activeStep <= steps.length && this.getStepContent(this.state.activeStep)}
			</Layout>
		);
	}
			...
			<Grid item xs={4}>
				<Paper elevation={1} className={classes.infoReservartion}>
					<Grid container>
						...
						<Grid item xs={4}>
							<Typography align="right" variant="subtitle1" component="p" gutterBottom>
								{this.props.price}€
							</Typography>
						</Grid>
						<Grid item xs={8}>
