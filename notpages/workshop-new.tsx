...
class WorkshopNew extends React.Component<IWorkshopNewProps> {
  ...
  public onSubmit(values, { setSubmitting, setErrors, setStatus }) {
    ...
    const createWorkshopError = result => {
      openSnackbar(
        "Échec lors de la création de votre atelier. Veuillez réessayer",
        "error"
      );
          ...
          {({ loading, error, data }) => {
            ...
            return (
              ...
              >
                ...
                <WizardForm.Page
                  validationSchema={Yup.object().shape({
                    name: Yup.string().required(
                      "Vous devez entrer un nom pour cet atelier"
                    ),
                    price: Yup.number()
                    ...
                    duration: Yup.number()
                      .required(
                        "Veuillez entrer la durée estimée de votre atelier en minutes"
                      )
                      .positive(),
                ...
                <WizardForm.Page
                  validationSchema={Yup.object().shape({
                    kitchenId: Yup.string().required(
                      "Vous devez nous proposer un lieu partenaire"
                    )
                  })}
                >
                  <Typography
                    ...
                    <Grid item xs={12}>
                      <Query query={GetKitchens}>
                        {({
                          ...
                          const kitchens =
                            kitchensData.getKitchens.kitchens.edges;
                          return (
                            <>
                              <Field
                                component={Select}
