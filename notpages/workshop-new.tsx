...
import KitchenIcon from 'mdi-material-ui/MapMarker';
import WorkshopIcon from 'mdi-material-ui/Silverware';
...
class WorkshopNew extends React.Component<IWorkshopNewProps> {
	...
	public onSubmit(values, { setSubmitting, setErrors, setStatus }) {
		...
		const { name, description, price, duration, date, gourmetRange, kitchenId } = values;
					...
					{({ loading, error, data }) => {
						...
						return (
							...
							>
								...
								>
									<Typography variant="h5" align="center" component="h1" color="inherit">
										Informations
									</Typography>
									<WorkshopForm />
									// @ts-ignore
										...
										<Grid item xs={12}>
											<Query query={GetKitchens}>
												{({ loading: kitchensLoading, error: kitchensError, data: kitchensData }) => {
													...
													return (
														<>
															<Field component={Select} name="kitchenId" label="Lieu partenaire" className={classes.textField} margin="normal">
															...
															<ErrorMessage name="kitchenId" component="div" className={classes.error} />
														</>
													);
												}}
											</Query>
...
const enhance = compose(withRedirect, graphql(CreateWorkshop), withStyles(styles as any));
